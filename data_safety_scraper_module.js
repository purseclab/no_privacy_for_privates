...
const MAPPINGS = {
  sharedData: {
    path: ['ds:3', 1, 2, 1, 138, 4, 0],
    fun: mapDataEntries
  },
  collectedData: {
    path: ['ds:3', 1, 2, 1, 138, 4, 1],
    fun: mapDataEntries
  },
  securityPractices: {
...
function mapDataEntries(block) {
  if (!block || !Array.isArray(block[0])) return [];

  const categories = block[0];

  return categories.flatMap(category => {
    const type = category?.[0]?.[1]; // e.g. "Personal info"

    const details = category?.[4];

    if (!Array.isArray(details)) return [];

    return details.map(detail => ({
      data: detail?.[0],
      optional: Boolean(detail?.[1]),
      purpose: detail?.[2],
      type
    }));
  });
